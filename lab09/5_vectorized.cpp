...
void mysgemm_vectorized(const float* __restrict__ A, const float* __restrict__ B, float* __restrict__ C, int M, int N, int K) {
    ...
    for (int k = 0; k < K; k += BK) {
        ...
        float4 tmp_a = reinterpret_cast<const float4*>(&A_ptr[load_a_row * K + load_a_col])[0];
        
        As[load_a_col + 0][load_a_row] = tmp_a.x;
        As[load_a_col + 1][load_a_row] = tmp_a.y;
        As[load_a_col + 2][load_a_row] = tmp_a.z;
        As[load_a_col + 3][load_a_row] = tmp_a.w;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Vectorized GEMM (Improved) M=" << M << " N=" << N << " K=" << K << std::endl;
    ...
    mysgemm_vectorized<<<grid, block>>>(d_A, d_B, d_C, M, N, K);
