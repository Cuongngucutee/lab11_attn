...
Tensor custom_paged_attn(
    ...
    #define LAUNCH_INSTANTIATED_KERNEL(H_SIZE, W_COUNT) \
       hipLaunchKernelGGL(( paged_attn_kernel_opt<H_SIZE, 16, W_COUNT>), dim3(grid), dim3(block), 0, stream,  \
    ...
    if (num_splits > 1) {
        ...
        if (head_size == 128)hipLaunchKernelGGL(( split_merge_kernel<128>), dim3(merge_grid), dim3(merge_block), 0, stream, (hip_bfloat16*)output.data_ptr(), (hip_bfloat16*)temp_acc.data_ptr(), temp_meta.data_ptr<float>(), num_splits, num_q_heads);
        else if (head_size == 256)hipLaunchKernelGGL(( split_merge_kernel<256>), dim3(merge_grid), dim3(merge_block), 0, stream, (hip_bfloat16*)output.data_ptr(), (hip_bfloat16*)temp_acc.data_ptr(), temp_meta.data_ptr<float>(), num_splits, num_q_heads);
    }
