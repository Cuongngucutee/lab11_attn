...
def bench_one_case(
    ...
    batch_size_label: int = 0,
    context_len_label: int = 0,
    log_file: str = None
):
    ...
    # --- LOGGING TO CSV ---
    if log_file:
        speedup = naive_us / custom_us if custom_us > 0 else 0
...
def main():
    ...
    # Define Log File
    log_file_name = "benchmark_results.csv"
    
    # Initialize Log File with Headers (Overwrite mode 'w')
    with open(log_file_name, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "Timestamp", "BatchSize", "ContextLen", "NumQHeads", 
            "NumKVHeads", "HeadSize", "Naive_Time_us", "Custom_Time_us", "Speedup"
        ])
